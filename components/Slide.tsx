...
const Room: React.FC<RoomProps> = ({ room, inventoryItems, visibleMapImages, className, overlayImageUrl }) => {
  const { image, backgroundColor } = room;

  const isLightBg = ['#ffffff', '#fbbf24', '#34d399'].includes(backgroundColor);
  ...
  return (
    ...
    >
      ...
      <div className="w-[30%] h-full flex flex-col bg-white/20 dark:bg-black/20">
        <div className="relative h-1/2 flex items-center justify-center text-center p-2 md:p-4 border-b border-slate-200 dark:border-slate-700">
