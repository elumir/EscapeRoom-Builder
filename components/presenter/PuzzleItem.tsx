...
const PuzzleItem: React.FC<{
    ...
    // === Hooks section: All hooks are declared here at the top level ===

    // Hooks for audio playback (for unsolved puzzles)
    const audioRef = useRef<HTMLAudioElement | null>(null);
    const [isPlaying, setIsPlaying] = useState(false);
    const [progress, setProgress] = useState(0);
    const [duration, setDuration] = useState(0);

    // Hook for flash effect (for newly solved puzzles)
    const [isFlashing, setIsFlashing] = useState(false);
    
    // === Effects section ===

    // Effect to handle the flash animation for newly solved puzzles.
    useEffect(() => {
        // Only flash when it becomes solved.
        if (puzzle.isSolved) {
            setIsFlashing(true);
    ...
    // Effect to handle audio setup and cleanup for unsolved puzzles.
    useEffect(() => {
        // If puzzle is solved or has no sound, we don't need audio.
        if (puzzle.isSolved || !puzzle.sound) {
            // Clean up any existing audio instance if state changes
            if (audioRef.current) {
    ...
    }, [puzzle.sound, puzzle.isSolved]); // Re-run if sound or solved state changes

    // Effect to pause audio if the puzzle becomes locked while playing.
    useEffect(() => {
