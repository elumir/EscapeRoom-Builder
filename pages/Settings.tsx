                                    ...
                                    <div className="grid grid-cols-2 gap-4 pt-2">
                                        <div>
                                            ...
                                            <div className="flex rounded-lg bg-slate-100 dark:bg-slate-800 p-1">
                                                <button onClick={() => handleGamePropertyChange('soundtrackMode', 'sequential')} className={`flex-1 text-center text-xs px-2 py-1 rounded-md transition-colors ${(game.soundtrackMode === 'sequential' || !game.soundtrackMode) ? 'bg-white dark:bg-slate-600 shadow-sm font-semibold' : 'text-slate-600 dark:text-slate-300 hover:bg-white/50 dark:hover:bg-slate-600/50'}`}>Sequential</button>
                                                <button onClick={() => handleGamePropertyChange('soundtrackMode', 'shuffle')} className={`flex-1 text-center text-xs px-2 py-1 rounded-md transition-colors ${game.soundtrackMode === 'shuffle' ? 'bg-white dark:bg-slate-600 shadow-sm font-semibold' : 'text-slate-600 dark:text-slate-300 hover:bg-white/50 dark:hover:bg-slate-600/50'}`}>Shuffle</button>
                                                <button onClick={() => handleGamePropertyChange('soundtrackMode', 'loop')} className={`flex-1 text-center text-xs px-2 py-1 rounded-md transition-colors ${game.soundtrackMode === 'loop' ? 'bg-white dark:bg-slate-600 shadow-sm font-semibold' : 'text-slate-600 dark:text-slate-300 hover:bg-white/50 dark:hover:bg-slate-600/50'}`}>Loop</button>
                                            </div>
                                        </div>
                                        <div>
